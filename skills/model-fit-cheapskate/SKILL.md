---
name: "model-fit-cheapskate"
description: "Explicit model-selection preflight only. Budget-minded advice for the whole submitted task; recommend available model/effort choices, then pause for confirmation."
---
...
State that confirmation covers the whole submitted task. Treat this latest card, its referenced requirements, and stop conditions as the pending agreement; no task-state file. Always stop here, including when keeping the current setting.
...
At completion, deliver the result and actual validation status. Add user-review items only when needed. Completion or pause grants no permission for another task.
